...
const useChatStore = create((set) => ({
	...
	createChat: async ({ chatName = "", memberIds, isGroup = false, profilPic = "" }) => {
		...
		try {
			...
			if (response.data.success) {
				...
				if (storedChat) {
					...
					throw new Error("Failed to retrieve stored chat from IndexedDB");
				}
			} else {
			...
			const errorMessage =
				err.response?.data?.message || err.message || "Network error occurred while creating chat";
