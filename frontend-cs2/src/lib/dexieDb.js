...
class ChatSecureDB extends Dexie {
	...
	async addChat(chatData, authUserId) {
		try {
			...
			if (!profilePic) {
				profilePic = chatData.isGroup ? "/group.png" : members[0]?.profilePic || "/user.png";
			}
			...
			let id;
			await this.transaction("rw", this.chats, this.chatMembers, async () => {
	...
	async syncChats(serverChats, authUserId) {
		return this.transaction("rw", this.chats, this.chatMembers, async () => {
	...
	async addMemberToChat(chatId, userId) {
		try {
			const existing = await this.chatMembers.where("[chatId+userId]").equals([chatId, userId]).first();
			if (!existing) {
	...
	async addMembersToChat(chatId, userIds) {
		try {
			...
			const toAdd = userIds.filter((id) => !existing.includes(id)).map((id) => ({ chatId, userId: id }));
			if (toAdd.length) {
				await this.chatMembers.bulkPut(toAdd);
	...
	async removeMemberFromChat(chatId, userId) {
		try {
			const rec = await this.chatMembers.where("[chatId+userId]").equals([chatId, userId]).first();
			if (rec) {
	...
	async getChatMembersWithDetails(chatId) {
		return this.transaction("r", this.chatMembers, this.users, async () => {
			const mems = await this.chatMembers.where("chatId").equals(chatId).toArray();
