...
class DataSync {
    ...
    async syncMissingUsers() {
        try {
            ...
            const cachedIds = await dexieDb.users
                .where("id")
                .anyOf(memberIds)
                .primaryKeys();
    ...
    async ensureUserKeys(authUserId) {
        ...
        for (const id of peers) {
            ...
            if (hasPendingKey)
                console.log("Key exchange already in progress for ", id);
    ...
    async fetchUndeliveredMessages() {
        try {
            ...
            console.error(
                "Failed to fetch undelivered messages and keys:",
                err
            );
        }
    }
        ...
        try {
            ...
            try {
                if (msg.iv) {
                    ...
                    if (key) {
                        plaintext = await EncryptionUtils.decrypt(
                ...
                console.error(
                    `Failed to decrypt message ${msg._id} from ${msg.senderId}:`,
                    decryptError
                );
