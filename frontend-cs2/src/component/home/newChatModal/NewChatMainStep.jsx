...
const NewChatMainStep = ({ chatState, updateChatState, onNext, onCreateChat, closeModal }) => {
	...
	useEffect(() => {
		...
		const timeoutId = setTimeout(async () => {
			if (trimmed.length > 2) setSearchResults(await searchUserName(trimmed));
			else {
				setSearchResults([]);
			}
		}, DEBOUNCE_DELAY);
	...
	const emptyMessage = search.trim().length === 0 ? "Type in the search bar to find users" : "No matching users";

	return (
		<div className='p-6'>
			<Header title={isGroupMode ? "Create Group Chat" : "Start Direct Message"} onClose={closeModal} />
		...
		<div className='flex items-center justify-center'>
			<div className='flex bg-gray-100 rounded-lg p-1'>
				{["DM", "Group Message"].map((mode, index) => (
					<button
						key={mode}
						onClick={() => (index === 1) !== isGroupMode && onToggle()}
		...
		<div className='flex flex-wrap gap-2'>
			{users.map((user) => (
				<div key={user._id} className='badge badge-primary gap-2'>
					{user.name}
					<button onClick={() => onRemove(user)} className='btn btn-xs btn-ghost btn-circle'>
						×
					</button>
				</div>
			))}
...
export const BottomButtons = ({ isGroupMode, canProceed, onCancel, onNext }) => (
